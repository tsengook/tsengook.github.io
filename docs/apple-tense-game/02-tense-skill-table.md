...
| T2 | Present Continuous | It is happening now. | 持续弹跳5秒（掉下来上去循环） | 弹簧音+位移拖影 | 正在进行 |
...
| T6 | Past Continuous | It was happening then. | 回放“当时正在反复弹跳” | 慢镜头上下回弹 | 过去某时正在进行 |
...
| T10 | Future Continuous | It will be happening. | 未来时段将持续上下折返 | 未来投影+循环回弹波纹 | 将来某时正在进行 |
...
## 反复掉落的喜剧参数（新增）
- 基础循环：`Drop(450ms) -> BounceUp(280ms) -> DropAgain(380ms)`。
- 默认循环次数：2 次；`Continuous` 系提升到 4 次。
- 夸张演出：每次反弹高度递减（100% -> 72% -> 48%），但表情崩坏递增。
- 音效建议：第一下“砰”、第二下“啵”、第三下“唉…”（形成反差喜感）。

## 叙事台词模板（可直接放进游戏）
- “你以为结束了？No. It is still happening.”
- “It had been falling… and somehow it bounced back again.”
- “By tomorrow, this apple will have bounced 27 times.”
